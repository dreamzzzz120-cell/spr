...
const sourceRoots = ['src', 'server.ts', 'server-entry.ts'];
...
const requiredReportProducts = [
  'Free Software Trust Snapshot',
  'Software Trust & Security Report',
  'MSP Client Security & Compliance Report',
  'Vendor Risk Portfolio',
  'Compliance Evidence Report',
  'Vulnerability Findings Report',
  '30-Day Audit Evidence Export',
  'MSP Executive Portfolio Report',
  'Passport Evidence Report',
  'MSP Co-Branded Trust Report',
  'Executive JSON Export',
  'Multi-Tenant Executive CSV',
  'Dynamic Compliance PDF',
  'Dynamic Compliance CSV',
];
...
if (!reportsView.includes('generateReportPDF')) errors.push('ReportsView.tsx: report templates are not wired to the canonical generator');

for (const product of requiredReportProducts) {
  if (!reportsView.includes(product) && !pdfGenerator.includes(product)) {
    errors.push(`report catalog: missing explicit product '${product}'`);
  }
}
...
console.log(`REPORT HARDENING GATE: PASS (${files.length} source files scanned)`);
