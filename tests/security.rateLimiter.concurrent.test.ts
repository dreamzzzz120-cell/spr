...
describe('security.rateLimiter provider adapters', () => {
  it('uses ioredis eval with window and limit arguments', async () => { const c = new MockEvalRedisClient(); const s = new RedisStore(new IORedisAtomicClient(c), c as any); await s.incr('rl:test', 2000, 5); expect(c.calls[0]).toMatchObject({ provider: 'ioredis', numKeys: 1, key: 'rl:test', limitStr: '5', windowMsStr: '2000' }); });
  it('rejects fail-open construction in production', () => { process.env.NODE_ENV = 'production'; expect(() => new RedisStore({ increment: async () => { throw new Error('redis'); } }, undefined, true)).toThrow(/cannot fail open/); });
  it('rejects unsupported atomic client configurations', () => { expect(() => createAtomicRateLimitClient('ioredis', { eval: undefined })).toThrow(); expect(() => (createAtomicRateLimitClient as any)('upstash', { eval: undefined })).toThrow(); });
  it('fails closed when REDIS_URL is missing in production', () => { process.env.NODE_ENV = 'production'; delete process.env.REDIS_URL; expect(() => createSharedRateLimitStoreFromEnv()).toThrow(/REDIS_URL is required in production/); });
  it('prevents test-only setters in production', () => { process.env.NODE_ENV = 'production'; expect(() => setRateLimiterConfig({ maxRequests: 1 })).toThrow(); expect(() => setRateLimiterStore({ incr: async () => ({ count: 1, resetAt: Date.now() + 1000 }) })).toThrow(); });
});
