...
export default function ReportsView({ clients = [] }: ReportsViewProps) {
  ...
  const generateAuditCsv = () => {
    ...
    void run('audit-30', async () => {
      const chain = await fetchAuditChain(); const cutoff = Date.now() - 30 * 24 * 60 * 60 * 1000;
  ...
  const generateJson = () => {
    ...
    void run('executive-json', async () => {
      const chain = await fetchAuditChain();
  ...
  const generateMultiTenantCsv = () => void run('multi-tenant-csv', () => {
  ...
  const generateComplianceCsv = () => {
    if (!selectedClient) return setError('Select a client before generating the compliance CSV.');
    void run('dynamic-csv', () => {
      const frameworks = selectedClient.complianceStatus || [];
      const rows = [['Framework Code', 'Framework', 'Progress', 'Compliant Controls', 'Total Controls', 'Reported Status'], ...frameworks.map(f => [f.code, f.name, f.progress, f.compliantControls, f.totalControls, f.status])];
      if (frameworks.length === 0) rows.push(['Not available', 'Not available', 'Not available', 'Not available', 'Not available', 'MISSING EVIDENCE']);
      downloadText(`dynamic-compliance-${slug(selectedClient.name)}.csv`, rows.map(row => row.map(csvCell).join(',')).join('\n'), 'text/csv;charset=utf-8');
    });
  };

  const generateProduct = (product: typeof REPORT_PRODUCTS[number]) => {
    if (product.kind === 'portfolio-csv') return generateMultiTenantCsv();
    ...
    if (product.kind === 'cobranded') {
      ...
      return void run(product.id, () => generateCoBrandedTrustReport(selectedClient, mspName.trim(), '#0f172a', reportTitle.trim() || 'Software Supply Chain Trust Report', undefined, undefined, undefined, true, true, true, true, true, selectedClient.softwareInventory?.map(item => item.name) || []));
    }
    if (!selectedClient) return setError('Select a client with persisted workspace data before generating a report.');
    return void run(product.id, () => generateReportPDF(selectedClient, product.template as 'rep-ceo' | 'rep-investor' | 'rep-auditor' | 'rep-vuln' | 'dynamic-compliance'));
  };
