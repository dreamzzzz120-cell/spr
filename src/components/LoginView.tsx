...
const authMessage = (error: any, fallback: string) => {
  switch (error?.code) {
    ...
    case 'auth/popup-blocked':
      return 'Your browser blocked the Google sign-in window. Try again.';
...
export default function LoginView({ onLoginSuccess }: LoginViewProps) {
  // Mock authentication is NEVER enabled merely because the app is in development.
  // It must be explicitly opted into so a real sign-in failure cannot be hidden by a fake session.
  ...
  const completeSignIn = async (firebaseUser: User) => {
    ...
    onLoginSuccess({
      ...
      displayName: firebaseUser.displayName || firebaseUser.email?.split('@')[0] || 'SPR user',
      token,
    ...
    getRedirectResult(auth)
      .then(async (result) => {
        ...
        try {
          ...
          if (active) setError(authMessage(err, 'Google sign-in could not be completed.'));
        } finally {
    ...
    try {
      if (useMockAuth) {
        const mock = {
          ...
          displayName: normalizedEmail.split('@')[0] || 'dev-user',
          token: 'mock-token-dev',
          emailVerified: true,
          onboarded: 0 as const,
        };
        onLoginSuccess(mock);
        return;
      }
      ...
      if (mode === 'signup') {
        ...
        if (!credential.user.emailVerified) {
          ...
          await signOut(auth);
          setNotice('Account created. Check your email and verify your address, then sign in.');
          setMode('login');
          setPassword('');
          setConfirmPassword('');
          return;
      ...
      // Do not allow an unverified password account into the protected workspace.
      if (!credential.user.emailVerified) {
        try {
          await sendEmailVerification(credential.user);
        } catch {
          // The account is still valid; verification can be retried later.
        }
        await signOut(auth);
        setNotice('Please verify your email before signing in. We sent a fresh verification email.');
        return;
      }
  ...
  const signInWithGoogle = async () => {
    ...
    try {
      ...
      const message = authMessage(err, 'Google sign-in failed. Please try again.');
    ...
    <main className="min-h-screen bg-slate-50 px-4 py-6 sm:px-6 lg:grid lg:grid-cols-[minmax(320px,0.9fr)_minmax(480px,1.1fr)] lg:gap-6 lg:p-6">
      <section className="relative hidden overflow-hidden rounded-[2rem] bg-slate-950 p-12 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <div className="relative">
          ...
          <div className="mt-20 max-w-lg">
            ...
            <p className="mt-6 max-w-md text-base leading-7 text-slate-400">Manage software passports, evidence, risk, and compliance from one secure workspace.</p>
          </div>
        </div>
      ...
      <section className="flex min-h-[calc(100vh-3rem)] items-center justify-center">
        <div className="w-full max-w-md py-8">
          ...
          <h2 className="mt-2 text-3xl font-semibold tracking-tight text-slate-900">{mode === 'login' ? 'Sign in to SPR' : 'Create your account'}</h2>
          <p className="mt-3 text-sm leading-6 text-slate-500">{mode === 'login' ? 'Use your work email or continue with Google.' : 'Create your account, verify your email, then sign in.'}</p>

          <div className="mt-8 grid grid-cols-2 rounded-xl bg-slate-100 p-1" aria-label="Authentication mode">
            {(['login', 'signup'] as const).map((item) => (
              <button key={item} type="button" onClick={() => switchMode(item)} disabled={loading}
          ...
          <form onSubmit={submitEmail} className="mt-6 space-y-5">
            ...
            <label className="block text-sm font-semibold text-slate-700">
              <span className="flex items-center justify-between">Password{mode === 'login' && <button type="button" onClick={resetPassword} disabled={loading} className="text-xs font-semibold text-indigo-600 hover:text-indigo-700">Forgot password?</button>}</span>
              <span className="relative mt-2 block">
                <input type={showPassword ? 'text' : 'password'} autoComplete={mode === 'login' ? 'current-password' : 'new-password'} value={password} onChange={(e) => setPassword(e.target.value)} disabled={loading} placeholder="At least 6 characters"
                ...
                <button type="button" onClick={() => setShowPassword((value) => !value)} aria-label={showPassword ? 'Hide password' : 'Show password'} className="absolute inset-y-0 right-0 px-4 text-slate-400 hover:text-slate-700">
            ...
            {mode === 'signup' && (
              <label className="block text-sm font-semibold text-slate-700">
                Confirm password
                <input type={showPassword ? 'text' : 'password'} autoComplete="new-password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} disabled={loading} placeholder="Repeat your password"
                  className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-normal text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10" />
              </label>
            )}

            <button type="submit" disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-3.5 text-sm font-semibold text-white shadow-lg shadow-indigo-600/15 transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-60">
          ...
          <button type="button" onClick={signInWithGoogle} disabled={loading} className="flex w-full items-center justify-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3.5 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-slate-50 disabled:opacity-60">
